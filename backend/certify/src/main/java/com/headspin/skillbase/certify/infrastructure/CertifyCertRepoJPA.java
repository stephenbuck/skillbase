package com.headspin.skillbase.certify.infrastructure;
...
public class CertifyCertRepoJPA implements CertifyCertRepo {
    ...
    @Transactional
    public void insert(
            @NotNull @Valid CertifyCert cert) {
        log.info("insert()");
        em.persist(cert);
    }
    ...
    @Override
    public Optional<CertifyCert> findById(
            @NotNull UUID id) {
    ...
    public List<CertifyCert> findAllBySkillId(
        ...
        return em.createQuery("SELECT c FROM certify_cert c WHERE c.skill_id = :skillId ORDER BY :sort", CertifyCert .class)
                .setParameter("sort", Objects.requireNonNullElse(sort, "id"))
                .setFirstResult(Objects.requireNonNullElse(offset, 0))
                .setMaxResults(Objects.requireNonNullElse(limit, 10))
                .getResultList();
    ...
    public List<CertifyCert> findAllByUserId(
        ...
        return em.createQuery("SELECT c FROM certify_cert c WHERE c.user_id = :userId ORDER BY :sort", CertifyCert .class)
                .setParameter("sort", Objects.requireNonNullElse(sort, "id"))
                .setFirstResult(Objects.requireNonNullElse(offset, 0))
                .setMaxResults(Objects.requireNonNullElse(limit, 10))
                .getResultList();
