...
public class MemberEventProducerKafka implements MemberProducerProvider {
    ...
    public MemberEventProducerKafka() {
        try {
            ...
            try (Admin admin = Admin.create(props)) {
                ...
                if (!listres.names().get().contains(MemberEvent.MEMBER_EVENT_TOPIC)) {
                    NewTopic topic = new NewTopic(MemberEvent.MEMBER_EVENT_TOPIC, 1, (short)1);
                    Set<NewTopic> topics = Collections.singleton(topic);
                    CreateTopicsResult result = admin.createTopics(topics);
                    KafkaFuture<Void> future = result.values().get(MemberEvent.MEMBER_EVENT_TOPIC);
                    future.get();
                }
            }
        }
    ...
    @Override
    @Transactional
    public void produce(MemberEvent e) {
        ...
        // Send the record
        producer.send(new ProducerRecord<>(MemberEvent.MEMBER_EVENT_TOPIC, event));
