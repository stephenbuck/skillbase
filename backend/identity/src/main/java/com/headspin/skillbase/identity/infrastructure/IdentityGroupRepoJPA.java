package com.headspin.skillbase.identity.infrastructure;
...
public class IdentityGroupRepoJPA implements IdentityGroupRepo {
    ...
    @Override
    public Optional<IdentityGroup> findById(
            @NotNull UUID id) {
    ...
    @Override
    public List<IdentityGroup> findAllByRoleId(
            @NotNull UUID roleId,
            @Null String sort,
            @Null Integer offset,
            @Null Integer limit) {
        log.info("findAllByRoleId(" + roleId + ")");
        return em.createQuery("SELECT g FROM identity_group g WHERE g.role_id = :userId ORDER BY :sort", IdentityGroup.class)
                .setParameter("sort", Objects.requireNonNullElse(sort, "id"))
                .setFirstResult(Objects.requireNonNullElse(offset, 0))
                .setMaxResults(Objects.requireNonNullElse(limit, 10))
                .getResultList();
    ...
    @Override
    public List<IdentityGroup> findAllByUserId(
            @NotNull UUID userId,
            @Null String sort,
            @Null Integer offset,
            @Null Integer limit) {
        log.info("findAllByUserId(" + userId + ")");
        return em.createQuery("SELECT g FROM identity_group g WHERE g.user_id = :userId ORDER BY :sort", IdentityGroup.class)
                .setParameter("sort", Objects.requireNonNullElse(sort, "id"))
                .setFirstResult(Objects.requireNonNullElse(offset, 0))
                .setMaxResults(Objects.requireNonNullElse(limit, 10))
                .getResultList();
